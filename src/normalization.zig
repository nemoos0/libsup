...
pub fn Normalizer(comptime form: Form) type {
    return struct {
        ...
        pub fn init(gpa: Allocator, source: code_point.Iterator) Allocator.Error!Self {
            return .{
                .gpa = gpa,
                .source = source,
        ...
        pub fn iterator(norm: *Self) code_point.Iterator {
...
fn expectEqualForm(
    ...
    var original_utf8: utf8.Decoder = .{ .bytes = original };
    var norm: Normalizer(form) = try .init(
        fba.allocator(),
        original_utf8.codeIterator(),
    );

    var expected_utf8: utf8.Decoder = .{ .bytes = expected };
